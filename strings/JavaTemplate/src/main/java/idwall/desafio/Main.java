...
public class Main {
	...
	public static void main(String[] args) {
		...
		if (justify == true) {
			...
			String[] just = outputText.split("\r\n");
			int length = 0;
			for (String j : just) {
				length = j.length();
				if (length < limit) {
					String[] palavra = j.split(" ", 2);
					int t = (limit - length) + 4;
					System.out.printf("%-" + t + "s", palavra[0].toString() + " ");
					System.out.printf("%" + t + "s", palavra[1].toString() + "\n");
				} else {
